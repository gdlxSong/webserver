...
////
////	//如果object = buffer
////	!!!对象池可以通过使用mutiset来进行先分配大的buffer. 但是这样的话buffer始终是在不断的长大的，直到达到临界值，而这时重新分配，那么有什么办法来缓解呢？
////
////*/
////
////
...
////		ObjectPool(size_t n = BASE_OBJECT_SIZE, Constructor && constructor = [] {return new ObjectClass; }) : idleCount(0), _cap(n), creator(constructor)
////		{
////			if (n > MAX_OBJECT_SIZE)
...
////			extend();
////#endif
////			cv.wait(lock, [this] { return size() > 0 || exited; });
////			try {
////			if (exited) return nullptr;
////
////			std::cout << "start get buffer...." << std::endl;
////
////			std::cout <<( pool.begin() == pool.end()) << std::endl;
////
////			
////
////
...
////					deletor(ptr);
////					}));
////				std::cout << "pool 生产啦...." << std::endl;
////				idleCount++;
////			}
////		}
////		~ObjectPool() { exited = true; cv.notify_all(); };
////		size_t size() { return idleCount; }
...
//	public:
//		using Constructor = std::function<T* (void)>;
//		using Element_type = std::unique_ptr<T, std::function<void(T*)>>;
//
//	public:
//		template<class ObjectClass = T>
//		ObjectPool(size_t n = BASE_OBJECT_SIZE, Constructor && constructor = [] {return new ObjectClass; }) : idleCount(0), _cap(n), creator(constructor)
...
//#endif
//			cv.wait(lock, [this] { return size() > 0 || exited; });
//			if (exited) return Element_type(nullptr);
//
//			Element_type uniqtr(pool.front(), [this](T* ptr) {
//				deletor(ptr);
//				});
//			pool.pop_front();
//			idleCount--;
